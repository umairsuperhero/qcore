...
**Current revision:** v1.24 — 2026-06-15
**Auditor of record this revision:** SUCI Profile A/B ECIES de-concealment on
`codex/suci-profile-ab`; Annex C.4 vector tests, Docker build/vet/test, Docker race
suite, dashboard `tsc`/Vite, and GitHub `ueransim-interop` run `27545087715`.
...
| Rev | Date | Summary |
|-----|------|---------|
| v1.24 | 2026-06-15 | **SUCI Profile A/B (ECIES) de-concealment is interop-validated for the bundled UERANSIM Profile-A path.** Added `pkg/suci` with stdlib-only TS 33.501 Annex C de-concealment: X25519 Profile A, P-256 Profile B, ANSI X9.63 KDF/SHA-256, AES-128-CTR, and HMAC-SHA-256 truncated to the 8-byte MAC tag. `pkg/suci` tests reproduce the official Annex C.4 Profile A and Profile B vectors before any UDM wiring. UDM now owns SIDF de-concealment from configured HN private keys (`udm.sidf_keys`); AMF passes concealed `suci-<hex>` through AUSF/UDM instead of rejecting non-null schemes. The bundled UERANSIM UE config uses Profile A with key ID 1, and the `ueransim-interop` workflow now gates on `SUCI PROFILE A PASS` from real collector events: run `27545087715` shows UERANSIM sent a concealed SUCI (106 hex chars), UDM de-concealed it to `imsi-001010000000001`, and the same run still prints `T10 DATA PLANE PASS` and `T10 SQN RESYNC PASS`. Verification for this revision: Docker focused package tests (`pkg/suci`, `pkg/config`, `pkg/udm`, `pkg/ausf`, `pkg/amf`, `pkg/diag`, `pkg/ai`), Docker `go build ./... && go vet ./... && go test ./...`, `make verify-fast`, dashboard `npm ci && npx tsc --noEmit --pretty false && npm run build`, and Docker `go build ./... && go vet ./... && go test -race ./...` passed locally; GitHub CI run `27545105015` and `ueransim-interop` run `27545087715` are green. Scope: Profiles A/B only, demo/local SIDF key management only; no carrier-scale key management and no broad real-device conformance claim. |
...
| 5G SA control plane | ✅ In-process E2E + bundled UERANSIM T10 replay pass | `pkg/amf` integration test green; UERANSIM completes registration and PDU session on cloud Linux (`27115478758`); Profile-A concealed SUCI registration de-conceals through UDM SIDF (`27545087715`) |
...
and the common UERANSIM test default), then layer **ECIES Profile A/B** behind the same
interface once external-device readiness demands it. Wire the simulator's configured IMSI
through it so the error scenarios test what they claim.
...
uesimtun0 8.8.8.8` succeeds in GitHub Actions run `27115478758`. That same interop gate
now also validates AUTS/SQN resync (`27529970131`) and concealed Profile-A SUCI
de-concealment (`27545087715`) for the bundled UERANSIM profile. C2/C3 credibility-gate
...
reconciliation is shipped. Phase 3 scenario authoring and CI hooks are shipped; the next
critical path is demand-driven per-target real-RAN replay, with Learning Mode lower
priority.
