...
**Current revision:** v1.22 — 2026-06-14
**Auditor of record this revision:** P3.1 scenario authoring on
`codex/e2-scenario-authoring` (implemented by Codex, verified by Claude); Docker
`go build`/`go vet`/`go test -race`, dashboard `tsc`/`vite build`, and a live 4G
dashboard runtime check of save→run→PASS/FAIL.
...
| Rev | Date | Summary |
|-----|------|---------|
| v1.22 | 2026-06-14 | **P3.1 scenario authoring complete.** Users can author, save, list, and re-run simulator scenarios with a deterministic PASS/FAIL + trace. Added `pkg/simulator` `ScenarioDefinition.Expect{result,cause,failed_step}` (optional pointer — back-compat: existing custom scenarios without `expect` still run, `pass=null`); `pkg/dashboard/scenario_store.go` (config-driven `dashboard.scenario_dir` YAML store, name-validated against path traversal, atomic tmp+rename writes); `pkg/dashboard/scenarios.go` (`GET/POST /api/scenarios`, `GET/DELETE /api/scenarios/{name}`, `POST /api/scenarios/{name}/run` with `CompareScenarioOutcome`); and a dashboard `ScenarioAuthoringPanel`. Implemented by Codex; checkpointed + verified by Claude after Codex hit its usage limit at the verification pass. **Dashboard runtime check** (local 4G stack via `make up`, all NFs healthy): a saved `result: success` happy scenario ran to `pass:true` (success/success, `j-6bcf79e1…`); a saved `wrong_ki` `result: failure` scenario ran to `pass:true` (failure/failure, `cause: wrong_ki`, `failed_step: security_mode_command`, `j-edb8c696…`). Verification: frontend `tsc --noEmit` + `vite build`, then — separately, to avoid the `dist` embed race — `go build ./... && go vet ./... && go test -race ./...` green in `golang:1.23`. Scope: dashboard/API authoring slice; P3.2 (CI hooks) + P3.3 (Learning Mode) remain. |
